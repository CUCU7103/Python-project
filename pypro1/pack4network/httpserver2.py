...
from http.server import CGIHTTPRequestHandler, HTTPServer # 웹 서버를 만드는데 필요하다.
...
class Handler(CGIHTTPRequestHandler): # 서블릿과 유사하다.  서블릿은 웹 요청과 응답의 흐름을 간단한 메서드 호출만으로 체계적으로 다룰 수 있게 해준다.
    cgi_directiries=['/cgi-bin'] # 서버와 연결할 파일의 경로 지정

     
serv = HTTPServer(('192.168.0.8',port),Handler)
print('웹 서비스 시작....')
serv.serve_forever() # 서버의 지속적인 실행을 위함.
